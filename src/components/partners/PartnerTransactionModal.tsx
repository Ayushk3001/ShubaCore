...
import { createPartnerTransactionAction } from "@/lib/actions";
...
export function PartnerTransactionModal({ partners, isOpen, onClose }: PartnerTransactionModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="w-full max-w-md rounded-xl border border-[#d8ded2] bg-white p-6 shadow-xl">
        <div className="flex items-center justify-between border-b border-[#edf1e8] pb-4">
          <h2 className="text-lg font-bold text-[#20231f]">Record Partner Capital Transaction</h2>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                name="type"
                required
                defaultValue="ADDITIONAL_INVESTMENT"
                className="mt-1 w-full rounded-lg border border-[#d8ded2] bg-[#fdfdfc] px-3 py-2 text-sm focus:border-[#3f563f] focus:outline-none"
              >
                {TYPES.map((t) => (
